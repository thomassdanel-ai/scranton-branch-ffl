...
// Lightweight mode: just { year } creates season row only (no leagues)
// Full mode: { year, numLeagues, leagueNames, rosterSize } creates season + leagues (backward compatible)
export async function POST(req: NextRequest) {
  try {
    ...
    // If league config is provided, validate it
    const hasLeagueConfig = numLeagues !== undefined && numLeagues > 0;
    if (hasLeagueConfig) {
      if (numLeagues < 1 || numLeagues > 4) {
    ...
    const { data: season, error: seasonErr } = await supabase
      ...
      .insert({
        ...
        year,
        status: 'setup',
        num_leagues: hasLeagueConfig ? numLeagues : 0,
        roster_size_per_league: hasLeagueConfig ? rosterSize : 0,
    ...
    // Create league rows only if full config was provided
    if (hasLeagueConfig && leagueNames) {
